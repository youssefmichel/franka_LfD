        ...
        // definition for FrankaModel
          ...
          dynamic_reconfigure::Server<franka_LfD::compliance_paramConfig>>(dynamic_reconfigure_compliance_param_node_);
        ...
        void CartesianImpedanceController::update(const ros::Time& /*time*/,
          ...
          pose_vector_[file_counter_].push_back(ros::Time::now().toSec()) ;
    ...
    //   position_d_ = filter_params_ * position_d_target_ + (1.0 - filter_params_) * position_d_;
      ...
      //visualize_act_pose(position) ;
        ...
        void CartesianImpedanceController::complianceParamCallback(franka_LfD::compliance_paramConfig& config,
            uint32_t /*level*/) {
            
          std::vector<float> temp  ;
          cartesian_stiffness_target_.setIdentity();
        
          if(! ros::param::get("/sitffness_params/stiffness_cart",temp)) {
          ROS_WARN("Could not find cartesian stiffness in yaml, using default values") ;
          cartesian_stiffness_target_.topLeftCorner(3, 3)
              << 2000 * Eigen::Matrix3d::Identity();

          }
          else{
            cartesian_stiffness_target_.topLeftCorner(3, 3)=Eigen::Vector3d(temp[0],temp[1],temp[2]).asDiagonal() ;
          }

          if(! ros::param::get("/sitffness_params/stiffness_orient",temp)) {
          ROS_WARN("Could not find orientation stiffness in yaml, using default values") ;
          cartesian_stiffness_target_.bottomRightCorner(3, 3)
              << 150 * Eigen::Matrix3d::Identity();

          }
          else{
            cartesian_stiffness_target_.bottomRightCorner(3, 3)=Eigen::Vector3d(temp[0],temp[1],temp[2]).asDiagonal() ;
          }

          cartesian_damping_target_.setIdentity();
          cartesian_damping_target_ = 2*0.7*cartesian_stiffness_target_.sqrt() ;
 
          if(! ros::param::get("/sitffness_params/stiffness_null",nullspace_stiffness_target_)) {
            
            ROS_WARN("Could not find null space stiffness in yaml, using default values") ;
